(function () {
	 ...
	 * @type {DefaultOptions}
	 * @name defaultOptions
	 * @property {Boolean} [useObsoleteKana=false] - Set to true to use obsolete characters, such as ヰ and ヱ.
	 * @example
	...
	// transform the tree, so that for example hepburnTree['ゔ']['ァ'][''] === 'va'
	// or kanaTree['k']['y']['a'][''] === 'キァ'
	 ...
	 * @example
	 * const sillyMap = createCustomMapping({ 'チァ': 'time', '茎': 'cookie'　});
	 * // sillyMap is passed defaultMapping to merge with when called in toRomaji()
	 * toRomaji("It's 茎 チァ ヨ", { customRomajiMapping: sillyMap });
	 * // => 'It's cookie time yo';
	 */
	...
	// NOTE: not exactly kunrei shiki, for example ヂァ -> dya instead of zya, to avoid name clashing
	/* eslint-disable */
	// prettier-ignore
	const BASIC_KUNREI = {
	  a: 'ァ', i: 'イ', u: 'ウ', e: 'エ', o: 'オ',
	  k: { a: 'ヵ', i: 'キ', u: 'ク', e: 'ケ', o: 'コ' },
	...
	const SMALL_Y = { ya: 'ァ', yi: 'ィ', yu: 'ュ', ye: 'ェ', yo: 'ョ' };
	const SMALL_VOWELS = { a: 'ァ', i: 'ィ', u: 'ゥ', e: 'ェ', o: 'ォ' };
	...
	const SPECIAL_CASES = {
	  ...
	  kwa: 'クァ',
	  whu: 'ウ',
	  // because it's not thya for テァ but tha
	  // and tha is not テァ, but テァ
	  tha: 'テァ',
	  thu: 'テュ',
	  tho: 'テョ',
	  dha: 'デァ',
	...
	const USE_OBSOLETE_KANA_MAP = createCustomMapping({ wi: 'ヰ', we: 'ヱ' });
	...
	// going with the intuitive (yet incorrect) solution where っヤ -> yya and っィ -> ii
	...
	function createKanaToHepburnMap() {
	  ...
	  Object.entries(YOON_EXCEPTIONS).forEach(([kana, roma]) => {
	    ...
	    // ジィ -> jyi, ジェ -> je
	    setTrans(`${kana}ィ`, `${roma}yi`);
	    setTrans(`${kana}ェ`, `${roma}e`);
	  });
